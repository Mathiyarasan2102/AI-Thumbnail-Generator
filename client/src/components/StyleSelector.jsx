...
const StyleSelector = ({ value, onChange, isOpen, setIsOpen }) => {
    ...
    const styleDescriptions = {
        ...
        "Tech/Futuristic": "Modern, sleak, tech-inspired",
    }
    ...
    return (
        <div ref={dropdownRef} className="relative space-y-3 dark">
            ...
            {isOpen && (
                <div className="absolute top-0 z-50 mt-26 w-full rounded-md border border-white/12 bg-black/20 backdrop-blur-3xl shadow-lg">
                    {thumbnailStyles.map((style) => (
                        <button type='button' key={style} onClick={() => { onChange(style); setIsOpen(false); }} className="flex w-full items-start gap-3 px-4 py-3 text-left transition hover:bg-black/30">
